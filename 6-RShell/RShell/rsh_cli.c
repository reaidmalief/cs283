    ...
    while (1) {
        ...
        if (strcmp(cmd_buff, EXIT_CMD) == 0 || strcmp(cmd_buff, "stop-server") == 0) {
            ...
            while (1) {
                ...
                printf("%.*s", (int)bytes_received - (rsp_buff[bytes_received-1] == RDSH_EOF_CHAR ? 1 : 0), 
                       rsp_buff);
            ...
            break; // Exit after receiving server response
        }
        ...
        while (!is_eof) {
            ...
            // Print all received bytes except the EOF character if present
            int print_bytes = bytes_received;
            if (is_eof) print_bytes--;
            
            if (print_bytes > 0) {
                printf("%.*s", print_bytes, rsp_buff);
                fflush(stdout);
            }
        }
    }
